...
# @Description :  This file represents daisy world for this model
...
def get_input():
    ...
    # NetLogo has 29 * 29 pathches, so the default number of X and Y 
    # are 28 seperately.
    parser.add_argument("--x", help="Num of X-coordinate of daisy world \
        (start from 0)", type=int, nargs='?', const=1, default=28,
        choices=range(0, 101))
    parser.add_argument("--y", help="Num of y-coordinate of daisy world \
        (start from 0)", type=int, nargs='?', const=1, default=28,
        choices=range(0, 101))
    parser.add_argument("--ticks", help="Total ticks, Default: Infinite", 
        type=float, nargs='?', const=1, default=float('inf'))
    parser.add_argument("--max_age", help="The max age of daisy", 
        type=int, nargs='?', const=1, default=25, choices=range(0, 1000))
    parser.add_argument("--start_whites", help="The percent of white daisies \
        when setup", type=int, nargs='?', const=1, default=20,
        choices=range(0, 50))
    parser.add_argument("--start_blacks", help="The percent of black daisies \
        when setup", type=int, nargs='?', const=1, default=20,
        choices=range(0, 50))
    parser.add_argument("--albedo_whites", help="The albedo of white daisies", 
        type=float, nargs='?', const=1, default=0.75)
    parser.add_argument("--albedo_blacks", help="The albedo of black daisies", 
        type=float, nargs='?', const=1, default=0.25)
    parser.add_argument("--scenario", help="scenario", nargs='?', const=1, 
        default="our-solar-luminosity", 
        choices=["ramp-up-ramp-down", "low-solar-luminosity", 
        "our-solar-luminosity", "high-solar-luminosity", 
        "maintain-current-luminosity"])
    parser.add_argument("--solar_luminosity", help="The solar luminosity you \
        would like to set. NOTE: This only works if the scenario \
        is maintain-current-luminosity", type=float, nargs='?', const=1, 
        default=1.00)
    parser.add_argument("--albedo_of_surface", help="The albedo of surface", 
    type=float, nargs='?', const=1, default=0.4)
    parser.add_argument("--pollution_level", help="The pollution_level is \
        extension part, which simulates the human's pollution. \
        It has 3 levels: 1-Low; 2-Medium; 3-High. The default value is 0 \
        namely no pollution", type=int, nargs='?', const=1, default=0)
    parser.add_argument("--pollution_frequency", help="How many ticks does \
        one pollution occur, default: 25 ticks", 
        type=int, nargs='?', const=1, default=25)
    args = parser.parse_args()
    return args

'''
This class represents the simulated world that this model runs on, 
which is in charge of the details of the simulation.
'''
class daisy_world(object):
    ...
    def __init__(self, args):
        ...
        # current_tick starts with 1 because the setup is treated as 0
        self.current_tick = 1 

    # Format the output and export the output to file for every tick
    def output(self, current_tick, white_num, black_num, 
        solar_luminosity, global_temperature, f):
        # Format the output
        fmt = "{:<10}\t{:<10}\t{:<10}\t{:<15}\t\t{:<20}"
        print(fmt.format(current_tick, white_num, black_num, 
            solar_luminosity, global_temperature))
        fmt = "{:<10}\t\t{:<10}\t{:<10}\t{:<15}\t\t{:<20}"
        f.write("\n" + fmt.format(current_tick, white_num, black_num, 
            solar_luminosity, global_temperature))

    # Format the output and export the output to file for once
    def output_one_time(self, f):
        ...
        general_info = "The range of X coordinate: (0 - " + str(self.x_max) + \
            ...
            str(self.pollution_level) + "    "
        if self.pollution_level != 0:
            general_info += "; Pollution frequency: every " + \
            str(self.pollution_frequency) + " ticks\n"
        else:
            general_info += '\n'
        print(general_info)
        ...
        print("\n" + fmt.format("Current_tick", "White_num","Black_num",
            "Solar_luminosity","Global_temperature"))
        f.write("\n")
        f.write(fmt.format("Current_tick", "White_num","Black_num",
        "Solar_luminosity","Global_temperature"))
    ...
    def diffuse(self):
        for x in range(MIN_XCOR, self.x_max + 1):
            for y in range(MIN_YCOR, self.y_max + 1):
                position = str(x) + "," + str(y)
                patch = patch_graph[position]
                diffuse_temperature = patch.get_temperature() * \
                                      self.DIFFUSE_PERCENT / 100
                patch.set_temperature(patch.get_temperature() - \
                                      diffuse_temperature)
                neighbors_list = self.get_neighbors(patch)
                for neighbor in neighbors_list:
                    neighbor.set_temperature(neighbor.get_temperature() + \
                    diffuse_temperature / len(neighbors_list))
    ...
    def seed_daisies_randomly(self):
        ...
        for white_count in range(0, white_daisies_num):
            empty_patch, empty_patch_list = \
            self.get_random_patch(empty_patch_list)
            if empty_patch != None:
    ...
    def calc_temperature(self):
        for x in range(MIN_XCOR, self.x_max + 1):
            for y in range(MIN_YCOR, self.y_max + 1):
                position = str(x) + "," + str(y)
                patch = patch_graph[position]
                patch.calculate_local_temperature(self.albedo_of_surface,\
    ...
    def setup(self):
        ...
        self.output(0, self.white_num, self.black_num, self.solar_luminosity,\
                    self.global_temperature, self.f)
    ...
    def pollution_simulation(self, pollution_level):
        ...
        for num in range(0, int(destroyed_num)):
            if len(patch_with_daisy_list) == 0:
                break
            random_patch, patch_with_daisy_list = \
            self.get_random_patch(patch_with_daisy_list)
    ...
    def update_every_tick(self):
        while(self.current_tick <= self.total_ticks):
            ...
            # in NetLogo
            self.cal_global_temperature()
            if self.pollution_level == 0 or (self.pollution_level != 0 and \
            self.current_tick % self.pollution_frequency != 0):
                self.white_num = self.get_daisy_num(Color.WHITE)
                self.black_num = self.get_daisy_num(Color.BLACK)

            self.output(self.current_tick, self.white_num, self.black_num, \
            self.solar_luminosity, self.global_temperature, self.f)
            ...
            if (self.scenario == "ramp-up-ramp-down"):
                if (self.current_tick > 200 and self.current_tick <= 400):
                    ...
                    self.solar_luminosity = float(format((\
                    self.solar_luminosity + 0.005), '.4f'))
                if (self.current_tick > 600 and self.current_tick <= 850):
                    self.solar_luminosity = float(format((\
                    self.solar_luminosity - 0.0025), '.4f'))
        self.f.close()

    # The function of starting simulation
    # provided to simulator.py 
